...
export class ComplaintComponent {
  ...
  InitializeViolationTable() {
    ...
    if (this._violationTable == undefined) {
          ...
          { data: 'correctiveaction' },
          {
            ...
            render: function (data: any, row: any, index: any) {
              ...
              if ((data != undefined) && (data != null))
                return self.getViolationStatusName(data);
              else
                return "";
            }
          },
    ...
    let but = "<button id=\"btnEdit\" class=\"btn custom-btn btn-sm\"(click)=editViolation(row) > Edit / View </button>";
    if (this._violationTable != undefined) {
      if (this._caseDetail.caseViolation != undefined) {
        this._violationTable.clear().rows.add(this._caseDetail.caseViolation).draw();
        this._violationTable.draw();
      }
    }
    ...
    // this._violationTable.on('click', 'button', function (e: any) {
    //   let data = self._violationTable.row(e.target.closest('tr')).data();
  ...
  actionOnChange(evt: any) {
    ...
    console.log('Item From Variable:' + this._actionLogViewModel.actionCode);
    let item: any = this._actionListDataList.filter((q) => q.actionCode == this._actionLogViewModel.actionCode);
  ...
  saveViolation() {
    try {
      ...
      if(this._caseViolationViewModel.duedate!=''){
        this._caseViolationViewModel.duedate+='T00:00:00';
      }
      if(this._caseViolationViewModel.opendate!=''){
        this._caseViolationViewModel.opendate+='T00:00:00';
      }
      if(this._caseViolationViewModel.closedate!=''){
        this._caseViolationViewModel.closedate+='T00:00:00';
  ...
  saveCaseDetail(status: string) {
    ...
    try {
      if ((!this.isCaseDetailProgramCode) && (!this.isCaseDetailPriorityCode) && (!this.isCaseDetailSourceCode)) {
        this._complaintServiceCall.saveByMethodName(this._urlConstant.CaseMasterModule, this._urlConstant.Save, cd).subscribe((response) => {
          if (response.status == "SUCCESS") {
            Lobibox.notify('success', {
              pauseDelayOnHover: true,
              continueDelayOnInactiveTab: false,
            ...
            this._caseDetailErrorSummary = JSON.parse(response.errorMessage.toString().replaceAll("\\u0027", "\"").replaceAll("[\"", "[").replaceAll("\"]", "]").replaceAll("\"{", "{").replaceAll("}\"", "}"));
            let errorMsg = '';
